...
export default class ViewAccountRecord extends LightningElement {
    ...
    get draftDetails() {
        if (! (this.recordDataExists && this.record.data.drafts && this.record.data.drafts.serverValues)) {
            return null
        }

        // flatten the serverValues
        const draftChanges = this.record.data.drafts.serverValues
        const serverChanges = Object.keys(draftChanges).map(key => ({ fieldName: key, ...draftChanges[key] }))
