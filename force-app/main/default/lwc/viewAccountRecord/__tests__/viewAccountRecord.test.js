...
describe('c-view-account-record', () => {
    ...
    it('should correctly populate record fields', () => {
        ...
        return Promise.resolve().then(() => {
            const objectField = element.shadowRoot.querySelector('lightning-layout-item[data-id="objectApiNameId"]')
            expect(objectField.textContent).toBe(element.objectApiName)

            const nameField = element.shadowRoot.querySelector('lightning-layout-item[data-id="nameId"]')
            expect(nameField.textContent).toBe(mockGetRecord.fields.Name.value)

            const iconField = element.shadowRoot.querySelector('lightning-icon[data-id="iconId"]')
            expect(iconField.iconName).toBe("standard:" + element.objectApiName.toLowerCase())
